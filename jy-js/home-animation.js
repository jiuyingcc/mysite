...
getHomeCases = () => {
	tpj.post({
		...
		success: (res) => {
			...
			typeStr = `
            <li class="activeFilter"><a style="padding: 10px 13px;" id="activeFilter" data-filter=".jyzqxxhxt" href="javaScript:void(0);">精选案例</a></li>`;
			// 案例列表渲染模板
	...
	tpj.post({
		...
		success: (res) => {
			...
			for (let key in res.data) {
				if (gameCaseType.includes(key)) {
					res.data[key].reverse();
				} else {
					delete res.data[key];
				}
			}
			let otherCaseList = res.data; // 案例类型列表数据
			let caseTypeList = Object.entries(otherCaseList).map(([key, value], index) => ({
				title: key,
				key: 'jytype' + index,
				value: value.slice(0, 9),
			})); // 改造数据结构
			caseTypeList.forEach((item) => {
				let caseListByType = item.value; // 案例列表
				// 分类列表渲染模板
				typeStr += ` <li class=""><a style="padding: 10px 13px;" onclick="showCase()" href="javaScript:void(0);" data-filter=".${item.key}">${item.title}</a></li>`;
				// 案例列表渲染模板
				caseListByType.forEach((caseitem) => {
					caseStr += `
                        <article class="portfolio-item col-lg-4 col-md-4 col-sm-6 col-12 pf-illustrations ${item.key} tempClass">
                            <div class="jy-article-img portfolio-image">
                                <a href="${caseitem.htmlUrl}">
                                    <img style="aspect-ratio: 3/2;" class="lozad" data-src="${caseitem.caseImg}">
                                </a>    
                            </div>
                            <div class="portfolio-desc">
                                <h4 style="font-weight: normal;">${caseitem.caseName}</h4>
                            </div>
                        </article>
                    `;
				});
